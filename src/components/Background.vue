...
<div class="overflow-x-hidden bg-gray-800">
  <div class="relative flex max-w-screen-xl flex-col overflow-hidden px-4 py-4 text-gray-200 md:mx-auto md:flex-row md:items-center">
    <h3 class="flex cursor-pointer items-center whitespace-nowrap text-2xl font-black text-blue-500">
    ...
    <div class="peer-checked:mt-8 peer-checked:max-h-56 flex max-h-0 w-full flex-col items-center justify-between overflow-hidden transition-all md:ml-24 md:max-h-full md:flex-row md:items-start">
		  ...
		  <a
			...
			:key="index"
			@mouseover="setImage(link.image)"
			@mouseleave="clearImage"
			@click="rotateBox(link.url)"
			href="#"
			class="nav-link"
		  >
			{{ link.name }}
  ...
  <section class="relative py-12 sm:py-16 lg:pt-20 xl:pb-0">
   ...
   <!-- Center Box -->
   <div class="flex-grow mx-auto flex justify-center items-center bg-transparent">
		<div
		  :class="['w-96 h-96 bg-white shadow-lg rounded-lg flex items-center justify-center transition-transform duration-500', { 'rotate-animation': isRotating }]"
		>
		  <img
			v-if="currentImage"
			:src="currentImage"
		  ...
		  />
		  <p v-else class="text-gray-500">Hover over a link to see an image</p>
		</div>
	  </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    navigateToProjects() {
      ...
      setTimeout(() => {
        this.$router.push('/projects'); // Navigate after animation
...
/* Slide In Animation for Project Page */
@keyframes slideIn {
  0% {
    transform: translateX(100%);
    opacity: 0;
